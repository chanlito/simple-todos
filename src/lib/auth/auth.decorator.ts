import { createRouteParamDecorator, ForbiddenException, ReflectMetadata, UnauthorizedException } from '@nestjs/common';
...
export function Auth(...roles: string[]): MethodDecorator {
  return ReflectMetadata('auth/ROLES_META_KEY', roles);
}

export const AuthUser = createRouteParamDecorator(async (args: { required: boolean }, req) => {
  if (!req.user) await authorizationChecker(req, [], args ? args.required : true);
  return req.user;
});
